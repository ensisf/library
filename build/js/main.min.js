...
var _pagesWidget = __webpack_require__(5);

var _pagesWidget2 = _interopRequireDefault(_pagesWidget);
...
(0, _pagesWidget2.default)(['index', 'about', 'for-authors', 'extended-search', 'competitions', 'play', 'person', 'author-catalog', 'play-catalog', 'contacts', 'blocks']);
...
jQuery(document).ready(function ($) {
  ...
  $('.js-toggle-nav').on('click', function (e) {
    ...
    _utils.$globals.body.toggleClass('navIn');
  });
...
var getElem = function getElem(selector, context) {
  var ctx = context || document;
  return ctx.querySelector(selector);
};

var getElems = function getElems(selector, context) {
  var ctx = context || document;
...
var setSvgIcon = function setSvgIcon(name, width, height) {
  var w = width || 30;
  var h = height || 30;
  return '<svg class="icon" width="' + w + '" height="' + h + '"><use xlink:href="#' + name + '" aria-hidden="true"/></svg>';
};

var getMax = function getMax(elems) {
  var max = elems.eq(0).outerHeight();
  elems.each(function (idx, elem) {
    max = $(elem).outerHeight() > max ? $(undefined).outerHeight() : max;
  });
  return max;
};
...
var declension = function declension(number, titles) {
...
var splitNumbers = function splitNumbers(number) {
  return number.toString().replace(/[^0-9]/gim, '').split('').reverse().reduce(function (prev, current, index) {
    return index % 3 === 0 ? prev + ' ' + current : prev + current;
  }, '').split('').reverse().join('');
};
